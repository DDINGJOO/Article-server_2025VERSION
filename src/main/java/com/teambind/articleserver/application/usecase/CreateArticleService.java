 ...
 * 게시글 생성 애플리케이션 서비스
 *
 * <p>Hexagonal Architecture의 Application Service입니다. Use Case를 구현하고 Port들을 조합하여 비즈니스 로직을 수행합니다.
 */
@Service
...
public class CreateArticleService implements CreateArticleUseCase {
  ...
  private Article createArticleByType(
      CreateArticleCommand command, Board board, List<Keyword> keywords) {
  ...
  private void publishArticleCreatedEvent(Article article) {
    PublishEventPort.ArticleCreatedEvent event =
        new PublishEventPort.ArticleCreatedEvent(
            article.getId(),
            article.getTitle(),
            article.getWriterId(),
