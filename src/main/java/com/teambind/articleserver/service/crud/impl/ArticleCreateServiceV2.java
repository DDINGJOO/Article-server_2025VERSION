 ...
 * 게시글 생성 서비스 V2
 *
 * Factory Pattern을 적용하여 OCP(Open-Closed Principle)를 준수합니다.
 * 새로운 게시글 타입 추가 시 기존 코드 수정 없이 확장 가능합니다.
 */
@Service
...
public class ArticleCreateServiceV2 {
     ...
     * 게시글 생성 (통합 메서드)
     *
     * 요청 정보를 기반으로 적절한 타입의 게시글을 생성합니다.
     *
    ...
    public Article createArticle(ArticleCreateRequest request) {
        ...
        log.info("Article created successfully - id: {}, type: {}, title: {}",
            savedArticle.getId(), type, savedArticle.getTitle());
    ...
    public Article updateArticle(String articleId, ArticleCreateRequest request) {
        ...
        if (article instanceof EventArticle eventArticle) {
            if (request.getEventStartDate() != null && request.getEventEndDate() != null) {
                eventArticle.updateEventPeriod(
    ...
    private ArticleType determineArticleType(ArticleCreateRequest request) {
        // 이벤트 기간이 있으면 EVENT
        boolean hasEventPeriod = request.getEventStartDate() != null
            && request.getEventEndDate() != null;

        // Board 정보 조회 (타입 결정을 위해)
        String boardName = null;
        if (request.getBoardIds() != null) {
            boardRepository.findById(request.getBoardIds())
                .ifPresent(board -> request.setBoardName(board.getName()));
            boardName = request.getBoardName();
        }

        return ArticleType.determineType(
            request.getBoardIds(),
            boardName,
            hasEventPeriod
        );
