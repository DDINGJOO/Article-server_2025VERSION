package com.teambind.articleserver.infrastructure.scheduler;
 ...
 * 게시글 정리 스케줄러
 *
 * <p>Hexagonal Architecture의 Infrastructure Layer입니다. 정기적으로 실행되는 배치 작업을 관리합니다. ShedLock을 통해 분산
 * 환경에서도 하나의 인스턴스만 실행되도록 보장합니다.
 */
@Component
@EnableScheduling
...
public class ArticleCleanupScheduler {
   ...
   * 삭제된 게시글 정리 작업
   *
   * <p>매일 새벽 4시 15분에 실행 DELETED 상태의 게시글을 DB에서 완전히 삭제
   *
   * <p>ShedLock 설정: - lockAtMostFor: 최대 9분간 락 유지 (작업이 비정상 종료되어도 9분 후 다른 인스턴스가 실행 가능) -
   * lockAtLeastFor: 최소 1분간 락 유지 (작업이 빨리 끝나도 1분 동안은 다른 인스턴스가 실행 불가)
   */
  @Scheduled(cron = "0 15 4 * * *")
  @SchedulerLock(name = "cleanUpDeletedArticles", lockAtMostFor = "9m", lockAtLeastFor = "1m")
